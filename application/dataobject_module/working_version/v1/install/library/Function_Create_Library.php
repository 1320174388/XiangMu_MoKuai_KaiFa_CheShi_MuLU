 ...
 */
Route::{$dataTypeL}(
    ':v/dataobject_module/{$name}_route',
    'dataobject_module/:v.controller.{$Name}Controller/{$name}{$DataTypeL}'
);
";
        // 判断文件是否存在
        if(file_exists("../route/dataobject_route_v1_api.php")){
            file_put_contents(
                '../route/dataobject_route_v1_api.php',
                $String, FILE_APPEND
            );
        }
    }
