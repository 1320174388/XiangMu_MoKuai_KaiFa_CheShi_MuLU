 ...
 */
Route::{$dataTypeL}(
    ':v/kalgorithm_module/{$name}_route',
    'kalgorithm_module/:v.controller.{$Name}Controller/{$name}{$DataTypeL}'
);
";
        // 判断文件是否存在
        if(file_exists("../route/kalgorithm_route_v1_api.php")){
            file_put_contents(
                '../route/kalgorithm_route_v1_api.php',
                $String, FILE_APPEND
            );
        }
    }
